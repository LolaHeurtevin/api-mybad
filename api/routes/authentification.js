...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await conn.execute('SELECT pseudo FROM User WHERE pseudo = ? AND password = ? AND isAdmin = 1', [req.body.pseudo, req.body.password]);
        ...
        return res.status(201).json({
            "_links": {
                ...
                "courts": hal.halLinkObject('/courts/{name}'),
                "reservations": hal.halLinkObject('/courts/{name}/reservations', 'string', 'Liste des réservations pour un terrain donné', true),
            },
            "access_token": token
        })
